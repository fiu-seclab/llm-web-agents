...
const generateAntiCaptchaTask = async (captcha, filedata) => {
    switch(captcha){
        case "textcaptcha":
            return {
        ...
        case "recaptchav3-enterprise":
            ...
            return {
                ...
                minScore: '0.7', // we want 0.5 minimum, but anticaptcha only accepts 0.3, 0.7, 0.9
                pageAction: 'login',
                isEnterprise,
            };
        case "cfturnstile":
        ...
        default:
            throw new Error(`generateAntiCaptchaTask: unsupported task type: ${captcha}`);
    }
};

const parseAntiCaptchaSolution = (captcha, solution) => {
    switch(captcha){
        case "textcaptcha":
        ...
        default:
            throw new Error(`parseAntiCaptchaSolution: unsupported task type: ${captcha}`);
    }
};

const reportAntiCaptchaAccuracy = async (captcha, taskId, success) => {
    switch(captcha){
        case "textcaptcha":
            if(success)
                return;

            const { data } = await api.post("/reportIncorrectImageCaptcha", {
                taskId
            });

            if(data.errorId)
                throw new Error(`Could not report anti-captcha incorrect captcha: ${JSON.stringify(data)}`);
        case "recaptchav2":
        case "recaptchav2-invis":
        case "recaptchav3":
        case "recaptchav3-enterprise": {
            const { data } = await api.post(success ? "/reportCorrectRecaptcha" : "/reportIncorrectRecaptcha", {
                taskId
            });
...
const doAntiCaptcha = async (db, captcha, filedata) => {
    const { data: taskData, status: taskStatus } = await api.post("/createTask", {
        task: await generateAntiCaptchaTask(captcha, filedata)
    });

    if((taskStatus < 200 || taskStatus >= 300) || !taskData.taskId){
        console.error(`Could not create anticaptcha task (status=${taskStatus})`, JSON.stringify(taskData));
        return null;
    }

    const { taskId } = taskData;
    const { lastID: solveId } = await db.run(`INSERT INTO solves (solver, captcha, task_id, text_filename, text_expected) VALUES ('anticaptcha', ?, ?, ?, ?)`, captcha, taskId, filedata?.filename ?? null, filedata?.expected ?? null);
    ...
    while(Date.now() - startTime < 120000){ // 2 minute timeout
        const { data: resultData, status: resultStatus } = await api.post("/getTaskResult", {
            taskId,
            softId: SOFTWARE_ID_ANTICAPTCHA
        });
        ...
        if(
            ...
            console.error(`Anticaptcha task failed (status=${resultStatus})`, JSON.stringify(resultData));
        ...
        if(resultData.status === "ready"){
            return [
                ...
                parseAntiCaptchaSolution(captcha, resultData.solution),
                reportAntiCaptchaAccuracy,
                taskId
            ];
        }
