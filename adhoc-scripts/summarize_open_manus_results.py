...
DEFAULT_RESULTS_DIR = (
    Path(__file__).resolve().parent / "open-manus-app" / "OpenManus" / "result"
)
...
def _detect_captcha(record):
    ...
    haystack = " ".join(
        str(record.get(k, ""))
        for k in ("final_assistant_text", "step_result", "result_preview")
    ).lower()
...
def _extract_tokens(record):
    for key in ("total_tokens", "token_total", "tokens_total"):
...
def _extract_cost(record):
    for key in ("total_cost", "cost_total", "usd_cost"):
        value = record.get(key)
        if isinstance(value, (int, float)):
            return float(value)
...
def _build_summary(records, max_observed_chars=0):
    if not records:
        return "No OpenManus records found.\n"
    ...
    for record in ordered:
        ...
        observed = str(record.get("final_assistant_text") or record.get("step_result") or "")
...
def main():
    parser = argparse.ArgumentParser(description="Summarize OpenManus captcha run results")
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=DEFAULT_RESULTS_DIR,
        help="Directory containing OpenManus *.jsonl result files",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    print(f"OpenManus summary written to: {args.output}")
